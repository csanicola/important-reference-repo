...
- the `predict()` function allows us to calculate the predicted y values
  - when used on a model with no data specified, it returns the predicted values for the training data
    - we can also specify new data using the `newdata` argument
      - the `newdata` must contain the columns given in the model formula
  - the `predict()` finds the predicted values, calculates the mean absolute error (MAE), and mean squared error (MSE)
    - the MAE is less sensitive to outliers compared to MSE
```{r}
...
### ----- Stepwise Selection -----
- when performing variable selection, there are multiple methods, one being the **stepwise selection**
- the `step()` function takes in an initial model along with a `direction`("forward", "backward", or "both"), and a `scope`
  - the **scope** specifies the lower and upper model formulas to consider
  - by default, the `step()` function prints each step in the process and uses AIC to guide its decision 
    - by setting the `trace=0`, we avoid the print behavior
  - updating the argument `k` to `log(n)` to use BIC where `n` is the number of observations
```{r}
lower_formula <- "log(SBP) ~ log(LEAD) + AGE + SEX:log(LEAD)"
upper_formula <- "log(SBP) ~ log(LEAD) + AGE + SEX:log(LEAD) + SEX + RACE + EDUCATION + SMOKE + INCOME + BMI_CAT"
mod_step <- step(model_interaction, direction = 'forward',
                 scope = list(lower = lower_formula,
                              upper = upper_formula))
# the final output is an lm object
```
```{r}
summary(mod_step)
# this summary of the final model shows that the adjusted R-squared has improved to 0.2479
```

## ----- Logistic Regression -----
- we will now look at the binary logistic regression using the `glm()` function
```{r}
...
- the `glm(formula, fata, family)` function is used to fit generalized linear models
...
- in binary logistic regression, we assume a binomial outcome and use the logit link function
  - we can specify this by setting `family = binomial`
    - by default this assumes the link function is the logit function
  - we can implement linear regression by setting `family = gaussian`

- we are now interested is current e-cig use
```{r}
nyts$e_cig_use <- as.factor(ifelse(nyts$num_e_cigs == 0, "0", "1")) # if the number of e-cigs is 0, then e-cig use is 0 but anything else is e-cig-use is 1 (positive)
table(nyts$e_cig_use)
```

- for the survey data, we are interested in the impact of taking the survey at school compared to other settings so we will simplify this variable
```{r}
table(nyts$location)
...
```{r}
mod_start <- glm(e_cig_use ~ grade + sex + race_and_ethnicity + location, family = binomial, data = nyts)

summary(mod_start)
```

- this time, we add the `exponentiate = TRUE` argument to exponentiate our coefficients to we can interpret them as estimated change in odds, rather than log odds
  - doing this we can see from the below that those who answered the survey at school have double the estimated odds of reporting e-cigarette use compared to those who took the survey at home/other, adjusting for grade, sex, race, and ethnicity
```{r}
tidy(mod_start, exponentiate = TRUE)
```
  
### ----- Residuals, Discrimination, and Calibration -----
- the `resid()` function finds the residuals of the distribution
  - you can specify by the Pearson and deviance residuals by changing the `type` argument
```{r}
par(mfrow=c(1,2))
hist(resid(mod_start, type = "pearson"), main = "Pearson Residuals")
hist(resid(mod_start, type = "deviance"), main = "Deviance Residuals")
# both of these plots, we can observe a multi-modal distribution, which reflects the binary nature of our outcomes
```

- now, we want to look at the predicted probabilities
  - by default, the `predict()` function returns the predicted value on the scale of the linear predictors
    - in this case, that is the predicted log odds
    - if we want to find the predicted probabilities, we update the argument by specifying `type="response"`
      - we can also predict on data not used to train the model by using the argument `newdata`
```{r}
pred_probs <- predict(mod_start, type = "response")
length(pred_probs)
```
- If we want to find the class for each observation used in fitting the model, we can use the model's output which stores the model matrix x and the outcome vector y
```{r}
# we can now plot the distribution of estimated probabilities for each class
ggplot() +
...
- we can now also plot the receiver operating characteristic (ROC) and compute the area under the curve (AUC)
  - the `roc()` function builds a ROC curve
    - you can specify the response vector `response` and predictor vector `predictor`
    - by default, with a 0/1 outcome, the `roc()` function assumes class 0 is controls and class 1 is cases
      - can also specify this in `levels` argument to set the value of the response for controls and cases
  - adding the `print.auc = TRUE` will add the AUC
```{r}
...
```
- to understand more about the curve, we can use the `coords()` function to find the coordinates for each threshold used to create the curve
  - the argument `x="all"` specifies that we want to find all threshold
    - can specify only to return the local maxima
```{r}
roc_vals <- coords(roc = roc_mod, x = "all")
head(roc_vals)
```
- with the above information, we can use this to find the highest threshold with a corresponding sensitivity above 0.75
  - this means that if we were to predict class 1 for all observations with a predicted probability above 0.062, then we would achieve a sensitivity of 0.77 and specificity of 0.56 on the training data
```{r}
roc_vals[roc_vals$sensitivity > 0.75, ] %>% tail(n=1)
```

- we can use the threshold of 0.080 indicated on our ROC curve to create predicted classes for the response
```{r}
...
#### ----- Calibration Plot -----
- another type of plot is a calibration plot
  - this type of plot groups the data by the estimated probabilities and compares the mean probability with the observed proportion of observations in class 1

- first we create a data frame with the predicted probabilities and the outcome variable
- then we group this data into `num_cuts` groups based on the predicted probabilities using the `cut()` function
- within each group, we find the model's predicted mean along with the observed proportion and estimated standard errors
...
- now we can plot the observed vs. expected proportions
- we also use the estimated standard error to create corresponding 95% confidence intervals
```{r}
ggplot(calib_data) +
...
- we are going to use the `step()` function to implement backward selection from a larger set of covariates
  - we first remove any observations with NA values to ensure that our training data does not change size as the formula changes
```{r}
nyts_sub <- nyts %>%
...
head(nyts_sub)
```
- now we only need to specify the lower formula in the scope
```{r}
...
```
- stepwise selection keeps most variables in the model and only drops family affluence
  - in the following output, we can see the AUC for this model has improved to 0.818
```{r}
...
- the `lrtest()` function allows us to input two nested `glm` models and performs a corresponding chi-square likelihood ratio test
```{r}
...
- now we try to fit a Poisson model to model the number of e-cigarettes used in the last 30 days by setting `family = poisson`
  - however, despite our outcome being a count value, this model does not appear to be a good fit for our data
```{r}
...
## ----- Model Selection -----
- expanding upon the stepwise selection by focusing on regularized regression
```{r}
...
- when referring to regularized regression, we are referring to solving the following optimization problem that minimizes the average loss plus a penalty term

### ----- Elastic Net -----
- the *glmnet* package implements elastic net regularizaation
```{r}
# we drop the first generated column which corresponds to the intercept
...
- by plotting the above object, we can see the model coefficients for each resulting model by plotting how the coefficient for each variable changes with the value or lambda
- the `label` argument adds a label to each line, though these are hard to read
  - the numbers at the top of the plot indicate how many non-zero coefficients were included in the model for different lambda values
```{r}
plot(mod_lasso, xvar = "lambda", label = TRUE)
```
- we can also print our results
  - this prints a matrix with the values of lambda used 
    - for each lambda value, we can also see the number of non-zero coefficients (`Df`) and the percent deviance explained (`%dev`)
```{r}
print(mod_lasso)
```
- we can extract the model for a particular value of lambda using the `coef()` function
  - the argument `s` specifies the value of lambda
    - for the particular value of lambda chosen in the following code, only age has a non-zero coefficient
      - the `coef()` function returns the coefficients on the original scale
```{r}
coef(mod_lasso, s = 0.045920)
```
- we can also use the `predict()` functoin to predict blood pressure for this particular model
  - in the function call, we have specified our value of lambda as well as our data matrix `x` as the data to predict on
```{r}
pred_lasso <- predict(mod_lasso, s = 0.45920, newx = x)
```
- this shows our observed model fit for a fairly penalty term
- in order to choose the best value of lambda, we use 5-fold cross-validation
- first, we randomly assign each observation to one of five folds using the `sample()` function
  - we can see that this splits the data into folds of roughly equal size
```{r}
set.seed(1)
foldid <- sample(1:5, size = nrow(x), replace = TRUE)
table(foldid)
```
- next, we call the `cv.glmnet()` function which implements k-fold cross-validation across a grid of lambda values
  - just like last time, we specified `x`, `y`, and `alpha = 1`
  - this time, we also include the measure to use for cross-validation (`mse` indicates mean squared error) and provide the fold vector `foldid`
    - if we do not want to provide folds, we can use the `nfolds` argument to specify the number of folds desired and the `cv.glmnet()` function
- plotting the returned object shows us the estimated mean squared error for different values of lambda as well as error bars for each estimate
  - it also shows the number of non-zero coefficients at the top
```{r}
cv_lasso <- cv.glmnet(x, y, foldid = foldid, alpha = 1,
                      type.measure = "mse")
plot(cv_lasso)
```
- there are two vertical dashed lines
  - these correspond to two values of lambda that are stored in our object
    - the first is `lambda.min` which is the lambda value with the lowest estimated mean squared error
    - the other is `lambda.1se` which is the largest lambda value whose estimated mean squared error is within one standard error of the lowest value
    
- we use the `lambda.min` value as our chosen lambda value
