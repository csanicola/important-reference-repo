...
There is also the ability to specify the vector for a distribution parameters to find the distribution function for different distributions.

```{r}
# if we want to find the probability function for X=4 for the distribution with p=0.25 and p=0.5
dbinom(4, size=10, p=c(0.25, 0.5))
```

### Cumulative Distribution

For the normal distribution, the cumulative distribution is give by `pnorm()`, which takes in a value `x`, a `mean`, and a `sd` and returns the probability that a random variable following a *N (mean, sd)* distribution is less than `x`.

-   For ex: `x` = to the mean which returns a 50% probability because the normal distribution is symmetric with mean equal to the median

    -   These are verified with two different values of the mean
...
The parameters for the distribution can be changed by passing a vector for `size` and/or `p` to the cumulative distribution function.

```{r}
# this will find the probability that x ≤ 3 and the probability that x ≤ 4 with 12 trials and a probability 0.25 and with 10 trials and and a probability 0.5
...
The quantile distribution function is the inverse of the cumulative distribution function.

The parameters are: `x`, a `mean`, and a `sd`

And returns the value for which the cumulative distribution function is equal to `x`

ex: when `x` = 0.5, the `qnorm()` function returns the median of the distribution, which is equal to the mean for the normal distribution
...
There are more probability distributions available in R. (Each is `r[dist]()` with `n` is the number of observations.
...
print(x)
# quantile distribution = qunif(x, min=x, max=x)
qunif(0.15, min=1, max=5)
```

## Empirical Distributions and Sampling Data

-   the `sample(x, size, replace=FALSE, prob=NULL)` function takes in:

    -   the values we want to sample from `x`

    -   the number of observations we want to sample `size`

    -   whether we want to sample with replacement `replace`

    -   if we don't want to sample where each value has an equal probability of being chosen, we can set a probability vector `prob`

        -   this must have the same length as `x`

```{r}
# here we sample 500 rows without replacement from `NHANESsample` data
...
```{r}
# now we are going to do sampling with replacement
# the new dataset is sampled from the empirical distribution of the data which is called a bootstrap sample
...
The `slice_sample()` function from **tidyverse** is another way to sample from a data frame.

-   you can specify the number of observations to sample with `n`

-   or the proportion of observations to sample `prop`

-   we can sample with or without replacement by setting the value of the argument `replace` with default value FALSE
...
set.seed(5)
x <- 1:100
practice_sample <- sample(x, 50, replace=TRUE)
mean(practice_sample)
```

\_\_\_

Empirical cumulative distribution -\> use a given vector to infer a distribution

-   use the function `ecdf()` for empirical cumulative distribution (which actually returns a function)

    -   this returned function can be used to find the sample cumulative distribution for different values similar to the `p[dist]()` function

```{r}
# here we draw a random sample from a normal distribution `vec` and find the empirical distribution using the `ecdf()` function
# we are looking to find the sample probability that x ≤ 0
vec <- rnorm(100)
ecdf_vec <- ecdf(vec)
ecdf_vec(0)
```

We can now plot the empirical distribution against the actual cdf using the `pnorm()` function

-   to do so, we need to create a sequence of possible `x` values to pass to both `pnorm()` and `ecdf_vec()`
...
For hypothesis testing, we will be comparing two samples and testing for any positive correlations, two-sample paired t-tests, and testing for equal variance among the groups.

The data is from the Texas Health and Human Services Department and includes the reported number of induced terminations of pregnancy (ITOPs) from 2016-2021, stratified by both race and county.

```{r}
library(tidyverse)
library(car) # this contained the `leveneTest()` function to implement a Levene's test for homogeneity of variance across groups
library(HDSinRdata)
library(gt)
library(gtsummary)
...
-   We want to look at a singular outcome of interest

```{r}
# our outcome of interest here is number of induced terminations of pregnancy (referred to as ITOPs or abortions below) in 2021 per 1000 females ages 15-49 in each county 
# we use the number of females as a proxy to scale the number of abortions to the population size, though this is not truly reflective of the number of people who can give birth in each county
county_rates_2021 <- tex_itop$total_rate[tex_itop$year == 2021]
hist(county_rates_2021, breaks=35)
```

-   from the output, we can see that the data is a very heavy-tailed distribution.

```{r}
# lets now look at the ten counties with the highest rates 
tex_itop %>%
  filter(year == 2021) %>%
...
-   looking at the top highest rates, we see some counties have small itop but high rates which indicates a small population but Harris, Bexar and Dallas have high rates and high itop meaning they have large population counts and high abortion numbers

    -   some of the counties are so small though, we may want to drop them as outliers (especially Loving); however, for the one-sample analysis, we do not remove them

```{r}
# lets look at the mean abortion rate among counties (the estimate from the CDC nationally is 11.2)
mean(county_rates_2021, na.rm=TRUE)
```

-   we can also calculate a confidence interval for the mean (for this case a 95% confidence interval)
...
-   we can also clean this up by using the `round()` function and display a text output
...
-   now, let's say we want to run a hypothesis test comparing the mean to another value -\> in this case we are going to look at the Texas Heartbeat Act which was introduced in 2021 and greatly reduced the number of eligible abortions, we could compare the two

    -   our null hypothesis will be mean \>= 6.23 (the mean abortion rate in 2020)

    -   we will run a one-sample t-test to test the hypothesis

        -   `x` = our sample

        -   `alternative` = type of test; in our case a one-sample t-test because the default is a two-sided test

        -   `mu`= the true value fo the mean (default 0)
...
-   based on the results, we can reject the null hypothesis and see that there was a drastic decline in abortion rates going from 6.23 -\> 5.16

-   since the output is just printed out, we want to save the results to use later:

    -   `t_test_res` is a list that contains information about the statistic, p-value, confidence interval, etc.

        -   you can see all thats contained by running `?t.test`

```{r}
t_test_res <- t.test(county_rates_2021, alternative="less", mu=6.23, conf.level=0.95)
names(t_test_res)
t_test_res$p.value
```
...
```{r}
county_rates_2019 <- tex_itop$total_itop[tex_itop$year == 2019]
mean(county_rates_2019, na.rm=TRUE)
hist(county_rates_2019, breaks=35)

t.test(county_rates_2019, alternative="less", mu=6.23, conf.level=0.95)
```

-   The thing about the `t.test()` function is that it assumes that the sample `x` comes from a normal distribution.
...
-   The one-sample Wilcoxon signed rank test is a non-parametric alternative to the one-sample t-test that can be used to compare the median value of a sample to a theoretical value without assuming that the data are normally distributed
    ...
    -   `t.test()` = equal distribution

    -   `wilcox.test()` = unequal distribution

```{r}
wilcox_res <- wilcox.test(county_rates_2021, alternative="less", mu=5.14, # 5.14 is the median rate from 2020 
                          conf.level = 0.95)
wilcox_res 
wilcox_res$p.value
```
...
-   now lets look at two-sample tests

```{r}
# we take the same 2020 and 2021 rates by county and pivot the data into a wider form to include rate columns and now we are going to also filter out the outliers
# if we first look at a scatter plot of the data, we can see a linear correlation between the two
...
-   we can also calculate the correlation between the two columns using the `cor()` function and the covariance using the `cov()` function
...
-   the is a positive correlation and the covariance is around 5.2

    -   now we can test to see if this correlation is significantly difference from zero

        -   the function `cor.test()` tests for association between paired samples

            -   it uses either Pearson's product moment correlation coefficient, Kendall's T, or Spearman's p

```{r}
# here we are testing whether there is a non-zero correlation between the 2020 and 2021 county rates using the Pearson's correlation
...
-   the above output shows that the resulting p means we can reject the null hypothesis that the correlation is zero and conclude that it is instead significantly different than zero

## Two-Sample Tests for Continuous Variables

-   Now, we want to compare the two samples with a two-sample paired t-test

-   we are testing the null hypothesis that the rates of the two are equal

```{r}
t_test_two_res <- t.test(x = county_rates$y2020,
                         y = county_rates$y2021)
t_test_two_res
# for this test we are keeping a few defaults:
# mu = 0
# alternative = "two.sided"
# var.equal = FALSE -> this is to say that we assume the samples have difference variances

t_test_two_res$conf.int
```

-   from this test, we can reject the null hypothesis that the two county rates are equal at the 0.05 levels

-   now if we want to look at another column in this dataset, the way that each county is categorized as either urban or rural -\> so now we can see if the rates change for urban or rural counties

```{r}
# first we create a variable describing the rate change between these years using the following code 
# we use the change in rate instead of percent change to avoid infinite or undefined values 
...
-   we are going to do a new two-sample test now but the data are not paired

    -   we are going to be looking at the `lhs` numeric column and `rhs` is a factor column with two levels
...
-   for this output, we would fail to reject the null hypothesis at the 0.05 level and conclude that the rate changes for urban and rural counties are not significantly different
...
# this is going to yeield the same results if we specify the vectors x and y

```

-   both the `t.test()` and `wilcox.test()` functions can only compare two groups, if we want to compare more than two groups, we can use a Kruskal-Wallis rank sum test using the `kruskal.test()` function or a one-way analysis of variance (ANOVA) using the `aov()` function

    -   for the `kruskal.test()`, we can either specify the arguments:

        -   `formula(rate_change ~ county_type)` and `data(county_rates_type)`

        -   or specify the two vectors: `x` - a numeric vector, and `g` - a factor representing the group
...
```{r}
# for the `aov()` function, we specify the test using a formula and the data (we need to use the `summary()` function to see the p-values)
aov_res <- aov(rate_change ~ county_type,
               data = county_rates_type)
summary(aov_res)
```

-   both of these results show that we fail to reject the null hypothesis
...
```{r}
county_rates_2016 <- tex_itop %>%
  filter(year == 2016) %>%
  pivot_longer(
    cols = matches("_(itop|rate)$"),   # select all count and rate columns
    names_to = c("race", ".value"),    # create two new columns: race and the "value" columns
    names_pattern = "(.*)_(itop|rate)" # extract race and metric type
  )

aov_res_2016 <- aov(itop ~ race,
                    data = county_rates_2016)
summary(aov_res_2016)
```

## Two-Sample Variance Tests

-   we can also test the variance of a continuous variable is equal between groups

-   we will be using the `var.test()` to implement the F-test and it uses the same arguments as the `t.test()`

    -   the default value for the `ratio` argument is 1, but you can always specify the hypothesized ratio

```{r}
# for this data, we will compare the variance in abortion rates in 2021 between urban and rural counties using an F-test
# the null hypothesis for this test is that the variance in both groups is equal
f_test <- var.test(y2021 ~ urban, county_rates_type)
f_test
f_test$estimate
```

-   from the output, we fail to reject the null hypothesis that the variances in rates are equal at the 0.05 level and when we print the estimate of the ratio of variance, its around 1.11
